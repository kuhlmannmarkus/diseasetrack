...
DB::DB(std::string _path, std::string _fileEnding)
    : m_path(_path), m_fileEnding(_fileEnding) {}
...
DB::getEncounters(std::string _puplicKeyContactPerson) {
  std::vector<std::tuple<std::string, std::string, UINT64>> result;

  std::string path = _puplicKeyContactPerson +
                     ".db"; // getFilenameOfContact(_puplicKeyContactPerson);
...
//############################################################################################################
std::vector<std::string> DB::splitString(std::string _stringToBeSplitted,
                                         std::string _delimeter) {
  std::vector<std::string> splittedString;
  size_t startIndex = 0;
  size_t endIndex = 0;
  while ((endIndex = _stringToBeSplitted.find(_delimeter, startIndex)) <
         _stringToBeSplitted.size()) {
    std::string val =
        _stringToBeSplitted.substr(startIndex, endIndex - startIndex);
...
std::vector<int> DB::obscure(std::string _pk) {
  ...
  std::vector<std::tuple<std::string, std::string, UINT64>> res =
      getEncounters(_pk);
